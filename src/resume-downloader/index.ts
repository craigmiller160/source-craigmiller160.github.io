...
const parseExperienceLine = (
  ...
  const currentExperienceHasAchievements =
    context.resume.experience[context.experienceIndex]?.achievements?.length >
    0;
  ...
  if (isPositionsLine) {
    ...
    return produce(context, (draft) => {
      draft.resume.experience[draft.experienceIndex].positions = positions;
    });
  }
...
const parseEducationLine = (context: ResumeParsingContext, line: string) => {
  ...
  const cleanedEducationLine = line
    ...
    .trim();
  const groups = ITEM_AND_INSTITUTION_REGEX.exec(cleanedEducationLine)?.groups;
  if (!groups) {
